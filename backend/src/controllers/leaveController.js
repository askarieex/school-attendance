...
const getLeave = async (req, res) => {
  try {
    ...
    // ✅ SECURITY FIX: Validate multi-tenancy - prevent School A from accessing School B's data
    ...
    if (studentCheck.rows.length === 0 || studentCheck.rows[0].school_id !== schoolId) {
      return sendError(res, 'Leave not found', 404);
    }
...
const updateLeave = async (req, res) => {
  try {
    ...
    // ✅ SECURITY FIX: Validate multi-tenancy before update
...
const deleteLeave = async (req, res) => {
  try {
    ...
    // ✅ SECURITY FIX: Validate multi-tenancy before delete
    const existingLeave = await Leave.findById(id);
...
const updateLeaveStatus = async (req, res) => {
  try {
    ...
    // ✅ SECURITY FIX: Validate multi-tenancy before status update
    const existingLeave = await Leave.findById(id);
